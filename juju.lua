...
do
	...
	pop_menu = LPH_JIT_MAX(function(a)
        ...
        context_action_service:BindActionAtPriority(
			...
			handle_click,
			false,
			100000, -- High priority to override Bloxstrike's weapon firing
		...
		if not menu_open then
			...
			-- Clean up the loop when the menu closes so the game can lock the mouse again
			if mouse_force_loop then
				mouse_force_loop:Disconnect()
				mouse_force_loop = nil
			end
		else
			-- Force unlock the mouse every frame while the UI is open
			if not mouse_force_loop then
				mouse_force_loop = run_service.RenderStepped:Connect(function()
